...
// Tujuan: saat link "?id=<slug>" ini dibuka oleh BOT pembuat thumbnail
// (WhatsApp, Telegram, Facebook, dll), balas dengan HTML kecil berisi judul
// & deskripsi TERBARU dari asesmen tsb, diambil langsung dari Supabase.
// Untuk pengunjung manusia biasa, middleware ini tidak melakukan apa-apa —
// permintaan diteruskan ke aplikasi React (SPA) seperti biasa.
...
export default async function middleware(request) {
  ...
  if (assessmentId) {
    try {
      const key = ASSESSMENT_PREFIX + assessmentId;
      const res = await fetch(
        `${supabaseUrl}/rest/v1/kv_store?key=eq.${encodeURIComponent(key)}&shared=eq.true&select=value`,
